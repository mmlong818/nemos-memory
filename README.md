...
Nemos 的真实优势是查询速度、本机嵌入、结果污染控制，以及命中结果的来源和事实时间可追溯。它在约束、归属安全、否定、用户隔离、计划变化、多语言、规律和流程记忆上全部通过，且整轮没有运行错误。

当前短板同样明确：Nemos 的 Recall@5 和 Top-1 仍落后于 Mem0 与 LangMem。本轮发现的 5 个召回失败集中在四类根因：显式年份查询没有自动进入历史召回；办公地点被错误归一为居住地；密集文本抽取遗漏护照与紧急联系人；相对时间没有以事件发生时间为基准解析。另有一次长期事实更新虽然在 Top-5 命中，但旧相机仍排在首位。
