...
这不是对公开 LongMemEval 排行榜的直接对照：`oracle` 数据变体和统一外部回答模型构成了独立赛道。上表中的 [Mem0](https://github.com/mem0ai/mem0)、[LangMem](https://github.com/langchain-ai/langmem)、[Graphiti](https://github.com/getzep/graphiti) 和 [Hindsight](https://github.com/vectorize-io/hindsight) 均在本次相同数据、模型、参数和裁判协议下重新运行，结果只适用于该可复现协议。
