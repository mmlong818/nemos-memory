...
2026 年 7 月 25 日，当前版本在 `core-v1` 中文测试集上完成了 5 轮连续回归。测试包含 10 个场景、每轮 12 个查询；答案由预先冻结的别名规则评分，不使用模型裁判。

| 产品 | 版本与运行 | Recall@5 | MRR | Top-1 | 冲突安全 | 来源可见 | 事实时间可见 | 查询均值 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| **Nemos Memory** | **0.7.4-alpha.1 最新性能复测** | **100.0%** | **1.000** | **100.0%** | **100.0%** | 91.7% | **100.0%** | **345 ms** |

5 轮稳定性测试中，Recall@5、MRR、Top-1、冲突安全和事实时间可见均保持 100%。优化后的性能复测将平均查询时间从 5 轮基线的 1019 ms 降至 345 ms，精确结构化事实查询约为 2 至 6 ms。来源可见率在 75.0% 至 91.7% 之间波动，仍需继续统一来源展示口径。

### 历史横向基线

在修复上述问题前，我们还使用相同输入、顺序、模型和向量模型，对四个固定版本进行过一次同轮黑盒测试。每个场景都从空存储开始，每次最多返回 5 条结果。

| 产品 | 当时测试版本 | Recall@5 | MRR | Top-1 | 冲突安全 | 来源可见 | 事实时间可见 | 查询均值 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| Nemos Memory | 0.7.4-alpha.1 修复前基线 | 91.7% | 0.917 | 91.7% | 91.7% | 75.0% | **100.0%** | 1246 ms |
...
历史横向表用于说明问题来源，不能代表当前 Nemos Memory 的成绩，也不是通用排行榜。当前修复版尚未与其他产品重新进行同轮测试；样本规模也只有 12 个查询。现阶段可以确认的是：当前版本的基础召回和事实更新已连续通过小型回归集，时间与来源建模是 Nemos 的明确方向；更大规模中文对话、删除传播、长期衰减和成本测试仍需继续补充。
